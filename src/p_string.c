...
int setruntimebase(char *path)
{
    ...
    {
        strncpy(runtimebasedir,path,sizeof(runtimebasedir)-1);
        runtimebasedir[sizeof(runtimebasedir)-1]=0;
    }
    return 0;
}

int buildruntimepath(char *dest, unsigned int len, char *path)
{
    char source[400];
    if(dest==NULL || path==NULL || len==0)
        return -1;
    strncpy(source,path,sizeof(source)-1);
    source[sizeof(source)-1]=0;
    if(runtimebasedir[0]==0 || source[0]=='/' || (strlen(source)>1 && source[1]==':'))
    {
        strncpy(dest,source,len-1);
        dest[len-1]=0;
    }
    else
        ap_snprintf(dest,len,"%s/%s",runtimebasedir,source);
    return 0;
}
...
int loadlanguage(char *langf)
{
    ...
    buildruntimepath(lfname,sizeof(lfname),lfname);
    maxindex=0;
    langname[0]=0;
