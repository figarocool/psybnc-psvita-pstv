...
char *dccrootpath(void)
{
    if(dccdownloadpath[0]==0)
        return "downloads";
    return dccdownloadpath;
}

void dccuserpath(char *buf, int len, int usern)
{
    ap_snprintf(buf,len,"%s/USER%d",dccrootpath(),usern);
}

void dccfilepath(char *buf, int len, int usern, char *file)
{
    ap_snprintf(buf,len,"%s/USER%d/%s",dccrootpath(),usern,file);
}
...
int pdccfconnected(int usern)
{
    ...
    oldum=umask(0000);
    umask(0000);    
    mkdir(dccrootpath(), 0777 );
    dccuserpath(mypath,sizeof(mypath),usern);
    mkdir(mypath, 0777 );
    umask(oldum);
    unlink(pdcc->file);
...
int addpendingdcc(int usern, int type, char *host, int port, char *nick, char *file, char *newfile, unsigned long fsize, int eproto)
{
	...
	case PDC_SENDTORQ:
	    ...
	    if(check==NULL)
	    {
		dccfilepath(myfile,sizeof(myfile),usern,fil);
