...
int envenabled(char *name)
{
  ...
  if(*envvalue=='0')
      return 0;
  if(*envvalue=='n' || *envvalue=='N' || *envvalue=='f' || *envvalue=='F')
      return 0;
  return 1;
}

void initruntimepaths(void)
{
  char *envvalue;
  envvalue=getenv("PSYBNC_BASE_DIR");
  if(envvalue!=NULL && *envvalue!=0)
      setruntimebase(envvalue);
  envvalue=getenv("PSYBNC_DOWNLOAD_DIR");
  if(envvalue!=NULL && *envvalue!=0)
      strmncpy(dccdownloadpath,envvalue,sizeof(dccdownloadpath));
  envvalue=getenv("PSYBNC_PID_FILE");
  if(envvalue!=NULL)
      strmncpy(pidfilename,envvalue,sizeof(pidfilename));
  envvalue=getenv("PSYBNC_LOG_FILE");
  if(envvalue!=NULL && *envvalue!=0)
      strmncpy(logfile,envvalue,sizeof(logfile));
}
...
main (int argc, char **argv)
{
  ...
  if(argc==2)
  {
      ...
      envvalue=getenv("PSYBNC_CONFIG_FILE");
      if(envvalue!=NULL && *envvalue!=0)
          strmncpy(configfile,envvalue,sizeof(configfile));
      else
          buildruntimepath(configfile,sizeof(configfile),"psybnc.conf");  /* rcsid */
  }
  ...
  fclose(conffile);
  foregroundmode=envenabled("PSYBNC_NOFORK");
  readconfig();
  ...
  if(envvalue!=NULL && *envvalue!=0)
      ...
      buildruntimepath(logfile,sizeof(logfile),logfile);
  oldfile(logfile);
